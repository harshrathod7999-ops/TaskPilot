"""The TaskPilot agent as a LangGraph state graph.

Flow:  plan → (reason → act → reason …) → critic → END
                     │                        │
                     └→ approval (interrupt) ─┘   (write actions only)

Design choices that matter in interviews:
  * **Explicit state machine** (not a black-box agent) so every transition is
    inspectable and streamable as a live trace.
  * **Critic / self-correction** node can bounce a weak answer back once.
  * **Hard step limit** prevents runaway loops (a classic agent failure mode).
  * **Human-approval gate** via LangGraph ``interrupt()`` before any WRITE tool —
    the graph pauses, the UI asks, and we resume with ``Command(resume=...)``.
  * Tools, the LLM, and a tiny event bus are the only side-effects; everything
    else is pure state transitions.
"""
...
# ---- registry singleton (tools are stateless; build once, reuse) ----------
...
async def _llm(messages: list[dict], *, max_tokens: int = 900, temperature: float = 0.2) -> str:
    s = get_settings()
    t0 = time.monotonic()
    # Emit a status so the UI shows "Waiting for provider…" during backoff/fallback.
    ...
    result = await asyncio.to_thread(
        lambda: chat(
            ...
            # llm.py also tries FALLBACK_MODEL_2 (Flash Lite) as a 3rd provider,
            # reading it from the env var; config exposes it for health checks.
            temperature=temperature, max_tokens=max_tokens,
            metadata={"trace_name": "taskpilot-agent"},
        )
    )
...
# Catches an unfilled template placeholder copied verbatim into a tool call —
# e.g. a model writes "[Winner Name]" or "{{name}}" into add_task's notes
# instead of resolving it to the real value it found (or should have gone
# and found). Caught here, before dispatch, it's a cheap corrective nudge;
# caught only by a human at the approval gate, it's a junk write action.
#
# Bracket/angle forms require 2+ words — a real placeholder reads as a field
# description ("Winner Name", "due date"), whereas legitimate short bracketed
# content ("[urgent]", "list[index]", "PR #123 [wip]") is almost always one
# word. {{double braces}} are distinctive enough on their own to need no such
# guard — that syntax essentially never appears in genuine prose.
...
async def reason_node(state: AgentState) -> dict:
    ...
    # Step limit reached → compose a final answer from history and stop looping.
    if step >= max_steps:
    ...
    if not decision:
        # One corrective retry — models occasionally wrap or omit the JSON.
    ...
    if "action" in decision and isinstance(decision["action"], dict):
        ...
        # Guard against an UNRESOLVED PLACEHOLDER written straight into a real
        # tool call (e.g. notes="Winner: [Winner Name]") — a model sometimes
        # copies a bracketed placeholder from its own plan verbatim instead of
        # substituting the real value. One corrective nudge before dispatch;
        # for a write tool this is what stands between garbage args and an
        # actual junk row in the tracker (the human approval gate is the
        # backstop, but catching it here means fewer bad approvals to review).
        ...
        if bad_value:
            ...
            # If the retry still doesn't produce a valid action, proceed with
            # what we have rather than looping — approval is the backstop.
    ...
    # Guard against PREMATURE finish: the model tries to answer (or claims it's
    # stuck / "needs a URL") without ever having called a tool. Force one tool call.
    if not used_a_tool and not state.get("forced_finish"):
        ...
        if isinstance(act, dict) and act.get("tool"):
            ...
            return {"pending": {"tool": act["tool"], "args": act.get("args", {}) or {},
                                "thought": th}}
        # If it still won't act, fall through and finish with whatever we can.

    # Guard against IGNORING the critic's mandate: the critic just said "you
    # MUST call this tool now" (prompts.py's ⚠️ MANDATORY block), and instead
    # of complying the model tried to finish anyway — sometimes even
    # hallucinating an observation ("waiting for the read_url result...") that
    # never happened. One corrective nudge, quoting the mandate back verbatim,
    # before we give up and let it fall through to the empty-final fallback.
    ...
    # Never emit an empty final answer — synthesize from history as a fallback.
...
def _collect_sources(state: AgentState) -> list[dict]:
    """Derive a source list deterministically from the scratchpad.

    Deliberately NOT forced [n] citation markers: the final answer is produced
    by whichever of three free-tier models happened to be live, and layering a
    strict citation protocol on top of the existing JSON-decision protocol
    would make weaker fallback models drop the JSON shape entirely. A
    derived list ("pages the agent actually read / queries it actually ran")
    is honest and can't hallucinate a URL that was never fetched.
    """
    scratch = state.get("scratchpad", [])
    # Map URL -> title by scanning prior web_search observations for a line
    # mentioning that URL (the tool renders "N. Title\n<url>\n<snippet>").
    titles: dict[str, str] = {}
    for s in scratch:
...
async def approval_node(state: AgentState) -> dict:
    ...
    # Pause the whole graph until a human responds (resumed via Command(resume=...)).
    # The API emits `approval_required` from the interrupt payload (once per pause),
    # so we don't emit it here — this node re-runs on resume and would duplicate it.
    # The extra fields (thought/step/schema) let the UI show context and render
    # a per-field edit form instead of a raw JSON textarea.
    decision = interrupt({
        "tool": tool_name,
    ...
    # Rejected: record it so the agent doesn't retry, and go back to reasoning.
...
async def critic_node(state: AgentState) -> dict:
    # Don't critique a forced (out-of-steps) finish, or beyond the critique budget.
...
async def finalize_node(state: AgentState) -> dict:
    """Re-render the critic-accepted draft as streamed, polished markdown.

    Reason's ``final_answer`` is a field inside a strict-JSON decision — that
    JSON-output constraint fights with natural prose formatting, and being a
    single field, it can't be streamed as it's produced. This dedicated node
    hands the accepted draft to one more LLM call whose only job is
    presentation, streamed token-by-token so the UI shows real generation
    instead of one block landing all at once. On any stream failure it falls
    back to the draft verbatim — presentation must never fail a run whose
    ...
    def produce() -> None:
        # Runs in a worker thread — chat_stream() is a blocking generator.
        # The stream writer (_emit) must only ever be called from the node's
        # own async task, so this thread only ever touches the queue.
        try:
            for tok in chat_stream(
                messages, model=s.primary_model, fallback_model=s.fallback_model,
                temperature=0.2, max_tokens=900,
                metadata={"trace_name": "taskpilot-finalize"},
            ):
                loop.call_soon_threadsafe(queue.put_nowait, tok)
        except Exception as e:  # noqa: BLE001 - surfaced via the queue, not raised here
...
def build_graph(checkpointer: BaseCheckpointSaver):
    """Assemble the graph. Checkpointer is injected so tests/eval can pass a
    fast in-memory ``MemorySaver()`` while the API uses a durable SQLite one."""
...
async def get_graph():
    """Lazily build the graph on a durable AsyncSqliteSaver.

    A plain long-lived aiosqlite connection (not the ``from_conn_string``
    context-manager form) is used deliberately: the context-manager form ties
    the connection's cancel scope to whichever asyncio task opens it, which
    breaks the moment a *different* request task tries to use the saver.
    A bare connection has no such constraint and is closed once, explicitly,
    from the FastAPI lifespan shutdown (see ``close_graph``).
    """
    global _graph, _agent_conn
    async with _graph_lock:
        if _graph is None:
            # agent.db is shared with runs_store.py's plain sqlite3 writes.
            # WAL + a generous busy_timeout let the two writers coexist
            # instead of one hitting "database is locked" on contention.
