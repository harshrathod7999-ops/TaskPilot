"""SQLite store behind the TaskPilot MCP server (the agent's 'action' surface).

Kept dependency-free and standalone so the MCP server process can run on its own.
The FastAPI app reads from the same DB file to render the task list in the UI.
"""
...
# backend/ root = parents[1] of this file (mcp_server/db.py).
_BACKEND_ROOT = Path(__file__).resolve().parents[1]


def db_path() -> Path:
    raw = os.getenv("TASKPILOT_DB", "tasks.db")
    p = Path(raw)
